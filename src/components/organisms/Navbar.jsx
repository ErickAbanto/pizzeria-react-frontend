...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-[#fff7e8] z-50 shadow-lg py-4 px-4 sm:px-6 flex justify-between items-center transition-all duration-300">

      {/* LOGO */}
      <Link to="/" className="flex items-center gap-3 min-w-0">
        <div className="w-[55px] h-[55px] sm:w-[60px] sm:h-[60px] bg-white rounded-full flex items-center justify-center border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] overflow-hidden transition-transform duration-300 hover:scale-105">
          <img src="/imgs/logo.png" alt="logo" className="w-12 h-12 object-contain" />
        </div>
        <span className="text-lg sm:text-xl font-bold tracking-wider text-black drop-shadow-[2px_2px_0px_rgba(0,0,0,1)] transition-all duration-300 hover:text-yellow-600 truncate">
          OHANA
        </span>
      </Link>
      ...
      <button
        ...
        className="lg:hidden w-11 h-11 sm:w-12 sm:h-12 flex items-center justify-center bg-white border-4 border-black rounded-full shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] active:scale-95 transition-all duration-300 relative"
      >
      ...
      {/* MENÚ DESKTOP */}
      <ul className="hidden lg:flex items-center gap-8 xl:gap-12 text-sm xl:text-base font-bold text-black">
      ...
      {openMenu && (
        <>
          ...
          <div
            ref={menuRef}
            className="fixed top-0 right-0 h-full w-[78%] sm:w-[60%] md:w-[45%] bg-[#fff7e8] lg:hidden border-l-4 border-black shadow-[-8px_0px_0px_0px_rgba(0,0,0,1)] flex flex-col py-8 px-6 gap-6 transition-all duration-300 animate-slideLeft z-50 overflow-y-auto"
          >
            ...
            <div className="flex flex-col gap-6 mt-2">
              <Link to="/" onClick={() => setOpenMenu(false)} className="flex items-center gap-3 text-lg font-bold hover:text-yellow-600"><FaHome size={22} /> Inicio</Link>
              <Link to="/menu" onClick={() => setOpenMenu(false)} className="flex items-center gap-3 text-lg font-bold hover:text-yellow-600"><FaPizzaSlice size={22} /> Menú</Link>
              <Link to="/promociones" onClick={() => setOpenMenu(false)} className="flex items-center gap-3 text-lg font-bold hover:text-yellow-600"><FaTags size={22} /> Promociones</Link>
              <Link to="/sobre-nosotros" onClick={() => setOpenMenu(false)} className="flex items-center gap-3 text-lg font-bold hover:text-yellow-600"><FaInfoCircle size={22} /> Sobre Nosotros</Link>
              <Link to="/contacto" onClick={() => setOpenMenu(false)} className="flex items-center gap-3 text-lg font-bold hover:text-yellow-600"><FaPhoneAlt size={22} /> Contacto</Link>
            </div>
